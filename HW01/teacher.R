...
setwd("D:/lin/BigData/data/TextMining/Reuters-21578")
...
#  docs <- VCorpus(DirSource(cname), readerControl=list(reader = readReut21578XML))
 docs <- VCorpus(DirSource(cname), readerControl=list(reader = readReut21578XMLasPlain))
docs
class(docs)
class(docs[[1]])
summary(docs)
...
## ------------------------------------------------------------------------
tmp <- read.csv("english.stop",header=F)
reuters.stop <- as.character(tmp$V1)

docs <- tm_map(docs, removeWords, c(reuters.stop,reuter) )
